...
(() => {
  ...
  async function diagnostics(){ const estimate=await storageEstimate(); const allLogs=getLogs(); const currentSessionLogs=allLogs.filter(entry=>String(entry.time||"")>=SESSION_STARTED_AT); return {generatedAt:new Date().toISOString(),format:"animus-diagnostics-v3",release:RELEASE,session:{startedAt:SESSION_STARTED_AT,currentLogCount:currentSessionLogs.length,historicalLogCount:allLogs.length-currentSessionLogs.length},url:location.href,userAgent:navigator.userAgent,viewport:{width:innerWidth,height:innerHeight,dpr:devicePixelRatio||1},online:navigator.onLine,document:{readyState:document.readyState,visibilityState:document.visibilityState},integrity:window.ANIMUS_INTEGRITY_RESULT||null,regression:window.ANIMUS_REGRESSION_STATUS||null,modules:moduleHealth(),performance:performanceSnapshot(),serviceWorker:{supported:"serviceWorker" in navigator,controller:!!navigator.serviceWorker?.controller,scriptURL:navigator.serviceWorker?.controller?.scriptURL||null,state:navigator.serviceWorker?.controller?.state||null},caches:await cacheInfo(),storage:{estimate,keys:Object.keys(localStorage).length,appDataPresent:!!localStorage.getItem(STORE)},database:{version:window.ACBF_DATABASE_VERSION,records:(window.ACBF_LOCATIONS||[]).length,issues:validateDatabase().length},logs:currentSessionLogs,historicalLogCount:allLogs.length-currentSessionLogs.length}; }
